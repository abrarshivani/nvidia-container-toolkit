...
# The bug that silently broke proxy resolution for all three uppercase modules.
...
assert_eq "tags.cncf.io/container-device-interface" \
    "$(proxy_escape tags.cncf.io/container-device-interface)" "proxy_escape leaves lowercase alone"

assert_eq "github.com/urfave/cli" \
    "$(strip_major_suffix github.com/urfave/cli/v3)" "strip /v3"
assert_eq "github.com/Masterminds/semver" \
    "$(strip_major_suffix github.com/Masterminds/semver/v3)" "strip /v3 below an uppercase org"
assert_eq "gopkg.in/yaml.v3" \
    "$(strip_major_suffix gopkg.in/yaml.v3)" "gopkg.in .vN is not a /vN suffix"

assert_eq "v2.0.1" "$(normalize_version 'v2.0.1+incompatible')" "strip +incompatible"
assert_eq "afd174a4e478" \
    "$(pseudo_version_hash v0.0.0-20260414002931-afd174a4e478)" "pseudo-version hash"
assert_eq "edf4cb3d2116" \
    "$(pseudo_version_hash v0.9.1-0.20251114084447-edf4cb3d2116)" "pre-release pseudo-version"
assert_eq "" "$(pseudo_version_hash v1.9.4)" "a tagged version has no hash"
...
assert_eq "" "$(github_repo_from_path tags.cncf.io/container-device-interface)" "non-github yields empty"
...
assert_eq "" "$(github_subdir_from_path github.com/urfave/cli/v3)" "/vN is not a subdir"
assert_eq "" "$(github_subdir_from_path github.com/sirupsen/logrus)" "root module has no subdir"

assert_eq "https://github.com/go-yaml/yaml" \
    "$(gopkg_in_repo gopkg.in/yaml.v3)" "gopkg.in single segment"
# The only gopkg.in module vendored here is gopkg.in/yaml.v3. The remaining two
# forms resolve differently, so they are covered with upstream module names.
assert_eq "https://github.com/go-inf/inf" \
    "$(gopkg_in_repo gopkg.in/inf.v0)" "gopkg.in single segment, no dash in repo"
...
assert_eq "specs-go" \
    "$(derived_subdir tags.cncf.io/container-device-interface/specs-go tags.cncf.io/container-device-interface)" \
    "subdir from prefix"
assert_eq "" "$(derived_subdir github.com/sirupsen/logrus github.com/sirupsen/logrus)" "root module"

assert_eq "https://github.com/sirupsen/logrus/blob/v1.9.4/LICENSE" \
    "$(blob_url https://github.com/sirupsen/logrus v1.9.4 LICENSE)" "github blob template"
assert_eq "https://go.googlesource.com/sys/+/refs/tags/v0.47.0/LICENSE" \
    "$(blob_url https://go.googlesource.com/sys refs/tags/v0.47.0 LICENSE)" "gerrit blob template"

assert_eq "https://raw.githubusercontent.com/stretchr/testify/v1.11.1/internal/spew/LICENSE" \
    "$(raw_url_for https://github.com/stretchr/testify/blob/v1.11.1/internal/spew/LICENSE)" \
    "github raw URL"
assert_eq "https://go.googlesource.com/sys/+/refs/tags/v0.47.0/LICENSE?format=TEXT" \
    "$(raw_url_for https://go.googlesource.com/sys/+/refs/tags/v0.47.0/LICENSE)" "gerrit raw URL"
